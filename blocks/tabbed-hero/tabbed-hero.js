export default function decorate(block) {
  ...
  // Create heading element - format with comma and line break
  const headingEl = document.createElement('h2');
  headingEl.className = 'tabbed-hero-heading';
  const firstHeading = tabsData[0]?.heading || '';
  headingEl.innerHTML = formatHeading(firstHeading);
  
  function formatHeading(text) {
    // Split at comma and add line break
    if (text.includes(',')) {
  ...
  tabsContainer.addEventListener('click', (e) => {
    ...
    // Update heading with formatting
    headingEl.innerHTML = formatHeading(tabsData[index]?.heading || '');
