export default function decorate(block) {
  // Block is already structured in HTML, just ensure classes are applied
  const heroSection = block.querySelector('.hero-section');
  const cardsGrid = block.querySelector('.cards-grid');
  
  // If the block doesn't have the expected structure, build it
  if (!heroSection) {
    ...
    if (cardRows.length > 0) {
      const newCardsGrid = document.createElement('div');
      newCardsGrid.classList.add('cards-grid');
      ...
      cardRows.forEach((row) => {
        ...
        newCardsGrid.appendChild(card);
        row.remove();
      });
      
      block.appendChild(newCardsGrid);
    }
  }
}